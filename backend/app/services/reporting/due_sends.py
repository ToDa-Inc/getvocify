...
def _already_sent(user_id: str, period_start: str, existing: list[dict]) -> bool:
    for row in existing:
...
def due_report_sends(now: datetime, people: list[dict], existing: list[dict]) -> list[dict]:
    """People due an email for today's local period who have not already received a sent delivery."""
    due: list[dict] = []
    for person in people:
        ...
        if _already_sent(person["user_id"], period_start, existing):
            continue
        due.append(person)
    return due
...
def run_due_reports(now: datetime, people: list[dict], existing: list[dict], send) -> None:
    """Invoke send once per due person; skips those already sent for the local period."""
