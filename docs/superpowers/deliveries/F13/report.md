...
`run_due_report_emails` enlaza `due_report_sends` con `send_report_email`; el bucle periódico de `main` invoca `tick_due_report_emails` como mucho una vez al día local de Madrid tras las 18:00 (`tests/reporting/test_tick_due_report_emails.py`, sin correo real verificado).
