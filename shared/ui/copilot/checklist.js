...
export function overlayChecklistMarkup(checklist, { kind } = {}) {
  ...
  const stepRows = steps
    .map((step) => {
      ...
      if (step?.status === "met") {
        return html`<p class="line overlay-checklist-step"><span>${label}</span> <span class="muted">Hecho</span></p>`;
      }
  ...
  return html`<div class="overlay-checklist">
    <p class="caps overlay-checklist-summary">${observed} de ${applicable}</p>
    ${stepRows}
  </div>`;
}
