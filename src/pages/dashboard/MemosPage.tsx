...
import { Mic, Search, Filter, Calendar, Clock, AlertCircle } from "lucide-react";
...
const MemosPage = () => {
  ...
  const fetchMemos = async () => {
    try {
      const data = await api.get<any[]>("/memos");
      setMemos(data);
      setError(null);
    } catch (err) {
  ...
  const filteredMemos = memos.filter(memo => {
    const company = memo.extraction?.companyName || "Untitled Company";
    ...
    <div className={`max-w-4xl mx-auto ${THEME_TOKENS.motion.fadeIn}`}>
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="w-12 h-12 rounded-full border border-border/40 flex items-center justify-center text-muted-foreground hover:text-beige hover:border-beige/50 transition-all shadow-soft">
