/**
 * Parse / normalize diarized transcripts for dashboard review.
 * Keep in lockstep with chrome-extension/lib/transcript-turns.js.
 */
...
export type SpeakerLabels = { s1?: string; s2?: string };

export function normalizeSpeaker(raw: string | null | undefined): string | null {
  const m = String(raw || "").match(/(\d+)/);
  return m ? `S${m[1]}` : raw ? String(raw).trim().toUpperCase() : null;
}

export function speakerSide(raw: string | null | undefined): "s1" | "s2" | "other" {
...
export function speakerDisplayLabel(raw: string | null | undefined, labels: SpeakerLabels = {}): string {
...
function fingerprint(turn: TranscriptTurn): string {
...
export function dedupeRepeatedConversation(turns: TranscriptTurn[]): TranscriptTurn[] {
...
export function mergeConsecutiveTurns(turns: TranscriptTurn[]): TranscriptTurn[] {
...
export function normalizeDiarizedTranscript(text: string): string {
...
export function reviewSpeakerLabels(contactName?: string | null): SpeakerLabels {
  return { s1: "You", s2: firstName(contactName) || "Them" };
}
